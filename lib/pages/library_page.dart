...
import '../data/comic_importer.dart';
...
class _LibraryPageState extends State<LibraryPage> {
  ...
  void initState() {
    super.initState();
    unawaited(_loadComicThumbnails());
  }

  /// Loads thumbnail URLs (and comic IDs) for all comics. If the user isn't
  /// signed in, navigates them to the sign-in page.
  Future<void> _loadComicThumbnails() async {
    ...
    try {
      ...
      if (mounted) {
        await _showErrorDialog(context, 'Failed to load comic thumbnails: $e');
      }
    }
  }
  ...
  Future<void> _pickAndImportComic() async {
    ...
    if (result != null && result.files.single.bytes != null) {
      ...
      try {
        ...
        if (context.mounted) {
          // Don't await - showDialog completes when dialog closes
          unawaited(
            _showProgressDialog(
              context,
        ...
        if (mounted) {
          await _showErrorDialog(context, 'Error importing comic: $e');
        }
      } finally {
  ...
  /// Deletes a single comic (Firestore + Storage), then refreshes the library
  /// UI.
  Future<void> _deleteComic(String comicId) async {
    ...
    try {
      ...
      if (mounted) {
        await _showErrorDialog(context, 'Error deleting comic: $e');
      }
    }
  }

  /// Shows a confirmation dialog before deleting the comic.
  Future<void> _showDeleteConfirmationDialog(String comicId) async {
    await showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        actions: <Widget>[
          ...
          TextButton(
            onPressed: () {
              Navigator.of(context).pop();
              unawaited(_deleteComic(comicId));
            },
            ...
            Positioned(
              ...
              child: DecoratedBox(
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    unawaited(_showDeleteConfirmationDialog(comicId));
                  },
                ),
              ),
            ),
